...
public class MainApp extends Application {
    ...
    private Model initModelManager(Storage storage, ReadOnlyUserPrefs userPrefs) {
        Optional<ReadOnlyStoreMando> addressBookOptional;
        ReadOnlyStoreMando initialData;
        try {
            addressBookOptional = storage.readStoreMando();
            if (!addressBookOptional.isPresent()) {
                logger.info("Data file not found. Will be starting with a sample StoreMando");
            }
            initialData = addressBookOptional.orElseGet(SampleDataUtil::getSampleStoreMando);
